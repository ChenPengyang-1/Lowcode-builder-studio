...
import type { MaterialType, PageNode, TemplateSummary } from '../types/schema';
...
function buildRecentDaySeries(templates: TemplateSummary[]) {
  ...
  return recentDays.map((date) => {
    ...
    const published = templates.filter((template) => {
      ...
      return publishedAt >= date && publishedAt < nextDate;
    }).length;
...
function buildNodeDistribution(nodes: PageNode[]) {
  ...
  return Object.entries(
    ...
    .map(([type, count]) => ({
      ...
      label: materialLabels[type as MaterialType] ?? type,
      count,
...
export function DashboardPage({ onNavigate }: DashboardPageProps) {
  ...
  const publishedTemplates = useMemo(
    () => templates.filter((template) => template.hasPublished),
    [templates],
  );
  ...
  const dashboardStats = useMemo(() => {
    const daySeries = buildRecentDaySeries(templates);
    const distribution = buildNodeDistribution(schema.nodes);
    const averageNodesPerTemplate = templates.length
      ? Math.round(templates.reduce((sum, template) => sum + template.draftNodeCount, 0) / templates.length)
      : 0;
  ...
  useEffect(() => {
    ...
    const syncWidth = () => {
      if (element.clientWidth > 0) {
        setMeasuredChartWidth(element.clientWidth);
      }
    };
    ...
    <div className="dashboard-page">
      <section className="dashboard-hero-card">
        <div>
          ...
          <h2>页面搭建与模板管理工作台</h2>
          <p>
            统一查看页面编辑、模板发布、资产复用和整体运行情况，方便把搭建过程、模板沉淀和数据概览串在一起。
          </p>
        </div>
      ...
      <section className="dashboard-metric-grid">
        <article className="dashboard-metric-card">
          ...
          <p>包括手工保存的模板、AI 生成后沉淀进模板中心的页面结果，以及导入模板。</p>
        </article>
        <article className="dashboard-metric-card">
          <span>已发布模板</span>
          <strong>{publishedTemplates.length}</strong>
          <p>发布后的模板可以在发布页继续预览、修改，或者重新载入编辑器继续迭代。</p>
        </article>
        <article className="dashboard-metric-card">
          <span>当前页面节点</span>
          <strong>{flattenNodes(schema.nodes).length}</strong>
          <p>当前页面结构统一由 Schema 管理，编辑态、发布态和 AI 修改结果都会复用同一套协议。</p>
        </article>
        <article className="dashboard-metric-card">
          <span>最近表单提交</span>
          <strong>{submissions.length}</strong>
          <p>预览态下的表单提交结果会先写入本地状态，方便调试和后续继续扩展。</p>
        </article>
      </section>
        ...
        <article className="dashboard-panel">
          ...
          <div className="dashboard-shortcuts">
            <button type="button" onClick={() => onNavigate('editor')}>
              ...
              <span>拖拽组件、编辑属性、导入导出 Schema，继续搭建当前页面。</span>
            </button>
            <button type="button" onClick={() => onNavigate('published')}>
            ...
            <button type="button" onClick={() => onNavigate('settings')}>
              ...
              <span>查看本地数据、主题风格、Schema 版本和当前项目状态。</span>
            </button>
          </div>
        </article>
            ...
            <div className="dashboard-template-list">
              {latestTemplates.map((template) => (
                <div key={template.id} className="dashboard-template-item">
                  ...
                  <span>{template.hasPublished ? '已发布版本' : '草稿版本'}</span>
